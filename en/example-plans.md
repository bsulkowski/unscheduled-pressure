# UP Tools — Example Plans

These examples show what a filled-in plan looks like for each of the three purpose-built UP tools. The activities and values are fictional but realistic.

---

## Pace Planner

Activities are grouped by their assigned daily average. The date shown is the current marker position relative to today (23 Apr 2026). A marker ahead of today means the activity has been getting more time than planned recently; behind today means it has been neglected.

**30 min**
- Cycling — 21 Apr

**20 min**
- Reading — 2 May

**15 min**
- Spanish — 19 Apr

**10 min**
- Piano — 25 Apr

**7 min**
- Sketching — 24 Apr

**5 min**
- Meditation — 23 Apr

*Today is 23 Apr. Spanish (19 Apr) and Cycling (21 Apr) are behind. Reading (2 May) and Piano (25 Apr) are ahead.*

---

## Recurring Tasks

Activities are grouped by their assigned interval. The date shown is when the activity was last completed. Within each group, a horizontal rule marks the point where the lower bound of the interval has been passed — activities below it are gently flagged.

**A few days**
- Take out bins — 21 Apr
- Water the plants — 20 Apr
- Check email backlog — 18 Apr

**10–20 days**
- Clean bathroom — 10 Apr

---
- Wipe down kitchen cupboards — 28 Mar

**1–2 months**
- Clean washing machine filter — 31 Mar
- Descale kettle — 4 Apr
- Car tyre pressure — 18 Apr

---
- Hoover under furniture — 8 Mar

**3–4 months**
- Check smoke alarms — 10 Feb
- Replace toothbrush — 15 Mar

---
- Clean extractor fan filter — 5 Jan

**6–8 months**
- Bleed radiators — 1 Nov 2025

---
- Clean behind the fridge — 12 Aug 2025

**1 year**
- Test RCD sockets — Apr 2025

---
- Service boiler — Oct 2024

---

## Scale-Free Planner

Plans are grouped by time slot, from the most immediate to the most distant. Each entry is one plan or project assigned to that slot.

**20–26 Apr**
- Home office repaint: prep & buy paint
- Anniversary trip: book restaurant

**27 Apr–3 May**
- Learn to drive: theory test

**1st half of May**
- Home office repaint: paint walls
- Learn to drive: practical lessons begin
- Bathroom renovation: get quotes
- Career change: update CV

**2nd half of May**
- (nothing currently assigned)

**June**
- Bathroom renovation: choose contractor

**Q3 (summer)**
- Bathroom renovation: works
- Career change: job search begins
- Weekend hiking habit: establish routine

**Q4 (autumn)**
- Learn to drive: practical test

**2027**
- Save for house: open savings account

**2028**
- Save for house: reach savings target

**2029–2032**
- Save for house: buy

*The first slots are concrete and actionable. The later ones are intentions — broad placeholders that will divide into narrower slots as they ripen.*
